...
enum StairDirection {Right, Left}

@export var descendo := true
@export var direction := StairDirection.Right
...
func _on_area_2d_body_entered(body: Node2D) -> void:	
	if body == player:		
		...
		GlobalData.set_player_spawn_position(player_spawn_position)		
